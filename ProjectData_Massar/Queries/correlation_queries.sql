...
-- Q: Is there a correlation between asteroid first date of observation and sex_or_gender in naming?
-- A: Earlier asteroids seem to be named after more females than males, but needs further (visual) examination
SELECT
	a.name as asteroid_name,
	STRING_AGG(n.name, ',') AS 'named_after',
	STRING_AGG(n.description, ',') AS description,
    a.first_observation,
...
-- Q: Get all asteroids with date of first observation and sex_or_gender for a boxplot diagram
--    (sometimes more than one named_after, therefore sex_and_gender needs to be aggregated)
-- A: see Figures/diameter_gender_boxplot.png and diameter_gender_correlation.py
SELECT
	a.spk_id,
